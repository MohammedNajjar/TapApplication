...
public class ROrderFragment extends Fragment {
    FragmentROrderBinding binding;
    Order currentOrder;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentROrderBinding.inflate(inflater, container, false);
        getActivity().setTitle("Add New Order");
        ...
        binding.emailResturentUser.setText(email);        setHasOptionsMenu(true);
...
//            binding.addNewOrderRadioGroup.setVisibility(View.VISIBLE);
//        }
        ((AppCompatActivity)getActivity()).setSupportActionBar(binding.detailsToolbar);

        getDataFromFirebaseITEM_CCOLLECTION();   // Method --> Fill the spinner through the firebase
        Intent intent = getActivity().getIntent();   // Receive from click at recycler view in main
    ...
    private void populateOrderItemsIntoRV() {
        ...
        binding.addNewOrderRv.setLayoutManager(new LinearLayoutManager(getActivity()));
    }
    ...
    private void fillOrderItemToFields(Order order) {
        ...
        if (order.getStatusPARKING().equals("Wifi")) {
            binding.addNewOrderRbWifi.setChecked(true);
        } else if (order.getStatusPARKING().equals("NONWifi")) {
            binding.addNewOrderRbNonWifi.setChecked(true);
        }

        if (order.getStatusWIFI().equals("Parking")) {
            binding.addNewOrderRbParking.setChecked(true);
        } else if (order.getStatusWIFI().equals("NONParking")) {
                ...
                if (orderId == null) {
                    ...
                    db.collection(ORDERS_COLLECTION).document(docID).set(order).addOnSuccessListener(new OnSuccessListener<Void>() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(getActivity(), "Added Failure", Toast.LENGTH_SHORT).show();
                        }
                    });
                } else {
                    ...
                    db.collection(ORDERS_COLLECTION).document(currentOrder.getId()).set(currentOrder).addOnSuccessListener(new OnSuccessListener<Void>() {
                        ...
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(getActivity(), "Updated Failure", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
                return true;
